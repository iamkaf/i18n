...
export async function requireTokenScopes(
  req: Request,
  needed: TokenScope[],
): Promise<{ tokenId: string; scopes: Set<string> }> {
  ...
  const list = (rows?.json ? JSON.parse(rows.json) : []) as Array<
    Pick<ApiTokenRow, "id" | "token_hash" | "scopes" | "disabled">
  >;
