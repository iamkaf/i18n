...
function ModerationStatusDot({ status }: { status: Suggestion["status"] }) {
  if (status === "pending") {
    return (
      <span
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center justify-center">
                                <ModerationStatusDot status={s.status} />
                            ...
                            <td className="px-4 py-2.5 text-[var(--atelier-text)] max-w-[14rem] truncate">
                              {s.source_string.source_text}
                            </td>
                            <td className="px-4 py-2.5 text-[var(--atelier-text)] max-w-[14rem] truncate">
                              {s.text}
                            </td>
                            <td className="px-4 py-2.5 text-xs text-[var(--atelier-muted)] whitespace-nowrap">
                              {s.author_name || s.author_discord_id}
                            </td>
                              ...
                              {s.status === "pending" && (
                                <div className="flex items-center justify-end gap-1">
                                  <Button
                                    ...
                                    variant="ghost"
                                    size="icon-xs"
                                    aria-label={`Approve ${s.source_string.key}`}
                                    title="Approve"
                                    className="text-emerald-600 hover:bg-emerald-500/10 hover:text-emerald-700 dark:text-emerald-400 dark:hover:bg-emerald-500/10 dark:hover:text-emerald-300"
                                    onClick={() => setDecision({ mode: "approve", suggestion: s })}
                                  >
                                    <Check />
                                  </Button>
                                  <Button
                                    ...
                                    variant="ghost"
                                    size="icon-xs"
                                    aria-label={`Reject ${s.source_string.key}`}
                                    title="Reject"
                                    className="text-rose-600 hover:bg-rose-500/10 hover:text-rose-700 dark:text-rose-400 dark:hover:bg-rose-500/10 dark:hover:text-rose-300"
                                    onClick={() => setDecision({ mode: "reject", suggestion: s })}
                                  >
                                    <X />
                                  </Button>
                                </div>
